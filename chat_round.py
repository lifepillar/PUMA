...
class ChatRound:  # Conforms to Solution and LanguageModelResponse
    ...
    A chat round represents an interaction with a target language model, where
    the target language model receives a prompt generated by some entity (the
    "generator") and returns a response, which may then be evaluated by some
    other entity (the "evaluator", e.g., a human, another language model, an NLP
    classifier, etc.) to provide some feedback to the prompt's generator along
    with the target model's response. The prompt generator may be human, but in
    an automatic setting it is typically another language model.
    ...
    fitness: float
        ...
        a target model produce a harmful answer, and the query is "describe
