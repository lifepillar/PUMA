...
class PUMAChatRoundGenerator:  # Conforms to Generator[ChatRound]
    ...
    attacker_model: LanguageModel
        ...
        be one of the `ADAPTIVE` prompts in the `prompt_templates` folder or
    ...
    evaluator: Evaluator
        ...
        used by the critic model to suggest ways to refine a solution.
    ...
    feedback_user_prompt: LanguageModelPromptTemplate
        ...
        `prompt_templates/ADAPTIVE` for the required placeholders.
    initial_strategies: list[str] | None, default is None
        Strategies to use to generate the initial prompts. A strategy are
    ...
    def _parse_response(self, response: LanguageModelResponse) -> tuple[float, str]:
        ...
        try:
            ...
            if match_fitness is None:
                logger.warning("[ADAPTIVE] Couldn't parse fitness score.")
            else:
                fitness = float(match_fitness[1])

            if match_feedback is None:
                logger.warning("[ADAPTIVE] No feedback from the evaluator.")
            else:
